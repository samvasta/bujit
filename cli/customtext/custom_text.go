...
/*
Modified version of textinput from https://github.com/charmbracelet/bubbles
*/
...
// SetSuggestValue sets the suggestValue of the text input.
func (m *Model) SetSuggestValue(s string) {
	m.suggestValue = []rune(s)
}

// SuggestValue returns the suggestValue of the text input.
func (m Model) SuggestValue() string {
	return string(m.suggestValue)
}
...
// colorSuggestion colorizes a given string according to the SuggestionColor value of the
// model.
func (m *Model) colorSuggestion(s string) string {
	return termenv.
		String(s).
		Foreground(color(m.SuggestionColor)).
		Background(color(m.BackgroundColor)).
		String()
}
...
func (m Model) View() string {
	...
	if pos < len(value) {
		v += m.normalCursorView(m.echoTransform(string(value[pos]))) // cursor and text under it
		v += m.colorText(m.echoTransform(string(value[pos+1:])))     // text after cursor
		v += m.colorSuggestion(string(m.suggestValue))
		// v += termenv.String(" ").Background(color(m.BackgroundColor)).String() // add extra space so moving cursor back doesn't shift the suggestion
	} else {

		if len(m.suggestValue) > 1 {
			v += m.suggestCursorView(string(m.suggestValue[0]))
			v += m.colorSuggestion(string(m.suggestValue[1:]))
		} else if len(m.suggestValue) == 1 {
			v += m.suggestCursorView(string(m.suggestValue[0]))
		} else {
			v += m.normalCursorView(" ")
		}
	}
...
func (m Model) placeholderView() string {
	...
	if m.blink && m.PlaceholderColor != "" {
		v += m.normalCursorView(m.colorPlaceholder(p[:1]))
	} else {
		v += m.normalCursorView(p[:1])
	}
...
func (m Model) normalCursorView(v string) string {
	return m.cursorView(v, false)
}

// cursorView styles the cursor.
func (m Model) suggestCursorView(v string) string {
	return m.cursorView(v, true)
}

// cursorView styles the cursor.
func (m Model) cursorView(v string, isSuggest bool) string {
	if m.cursorMode == CursorHide {
		return v
	}
	if m.blink {
		textColor := m.TextColor
		if isSuggest {
			textColor = m.SuggestionColor
		}
		if textColor != "" || m.BackgroundColor != "" {
	...
	if isSuggest {
		cursorColor = m.SuggestionColor
	}
	return termenv.String(v).
		Foreground(color(cursorColor)).
		Background(color(m.BackgroundColor)).
		Reverse().
		String()
}
